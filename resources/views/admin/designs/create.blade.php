...
<div class="container-fluid p-0">
    <div class="row mb-2 mb-xl-3">
        <div class="col-auto d-none d-sm-block">
            <h3><strong>Create</strong> Design</h3>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.designs.store') }}" method="POST" id="designForm">
                        @csrf
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="layout_id" id="layout_id" class="form-select @error('layout_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($layouts as $layout)
                                            <option value="{{ $layout->id }}" data-type="{{ $layout->type->name }}" {{ old('layout_id') == $layout->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="layout_type_id" id="layout_type_id" class="form-select @error('layout_type_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($layoutTypes as $type)
                                            <option value="{{ $type->id }}" {{ old('layout_type_id') == $type->id ? 'selected' : '' }}>
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="lang_code" id="lang_code" class="form-select @error('lang_code') is-invalid @enderror" required>
                                        ...
                                        @foreach($languages as $lang)
                                            <option value="{{ $lang->code }}" {{ old('lang_code') == $lang->code ? 'selected' : '' }}>
                                                {{ $lang->name }} ({{ $lang->code }})
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="data" id="data" class="form-control @error('data') is-invalid @enderror" 
                                      rows="15" placeholder="Enter section data in JSON format">{{ old('data') }}</textarea>
                            @error('data')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
